...
export default async function handler(req: any, res: any) {
  ...
  const { googleToken, title, content } = req.body ?? {};
  ...
  try {
    ...
    if (!createRes.ok) {
      ...
      console.error(JSON.stringify({ endpoint: '/api/docs', event: 'create_failed', status: createRes.status }));
    ...
    if (!updateRes.ok) {
      console.error(JSON.stringify({ endpoint: '/api/docs', event: 'update_failed', status: updateRes.status }));
    ...
    console.error(JSON.stringify({ endpoint: '/api/docs', event: 'error', message: err?.message }));
