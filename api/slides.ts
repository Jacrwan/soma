...
export default async function handler(req: any, res: any) {
  ...
  const { googleToken, title, slides } = req.body ?? {};
  ...
  try {
    // 1. Create the presentation (comes with one default blank slide)
    ...
    // 2. Build the batchUpdate: remove default slide, add a title slide + content slides
    const requests: any[] = [];
    if (defaultSlideId) {
    ...
    if (!updateRes.ok) {
      console.error(JSON.stringify({ endpoint: '/api/slides', event: 'update_failed', status: updateRes.status }));
      // Presentation was created but population failed — still return the URL so it isn't orphaned silently
    ...
    console.error(JSON.stringify({ endpoint: '/api/slides', event: 'error', message: err?.message }));
