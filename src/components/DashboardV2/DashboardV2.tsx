...
export interface DashboardRuntime {
 ...
 onState: (id: string | number, state: State, opts?: { deleteLoggedTime?: boolean }) => Promise<void>;
 ...
 /** `startTime` ("HH:MM") is set when the student gave a clock range rather
  *  than just a length; without it the entry falls back to the block's
  *  planned start. */
...
export default function DashboardV2({runtime}:{runtime?:DashboardRuntime}) {
  ...
  return <div className={`${styles.page} ${runtime ? styles.live : ""}`} data-time-format={timeFormat} data-theme={runtime ? undefined : "light"}>
    ...
    <main className={styles.main}>
      ...
      <section className={styles.section} data-tour="plan"><div className={styles.sectionHeading}><h2>{day===0 ? 'Today’s plan' : day<0 ? `${date.toLocaleDateString(undefined,{weekday:'long',month:'short',day:'numeric'})}` : 'Your plan'}</h2><div className={styles.planTools}>{runtime?.onAcceptAll && blocks.some(b=>b.state==='Proposal') && !editing && <button className={styles.acceptAll} onClick={()=>void runtime.onAcceptAll?.()}>Accept all ({blocks.filter(b=>b.state==='Proposal').length})</button>}{editing && <button onClick={()=>openEditor()}>+ Add block</button>}<button aria-pressed={editing} onClick={()=>{setEditing(!editing);setEditor(null);}}>{editing ? 'Done editing' : 'Edit plan'}</button></div></div><p className={styles.description}>A place for your work, and space between it.</p>
