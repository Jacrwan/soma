...
import { Check, Play, Pause, ArrowUpRight, CalendarDays, BookOpen } from 'lucide-react';
...
export default function DashboardV2({runtime}:{runtime?:DashboardRuntime}) {
  ...
  return <div className={`${styles.page} ${runtime ? styles.live : ""}`} data-theme={runtime ? undefined : "light"}>
    {!runtime && <aside className={styles.sidebar}><a className={styles.brand} href="/day-view">soma<span>study with intention</span></a><nav aria-label="Preview navigation"><span className={styles.selected}><CalendarDays size={17}/> Dashboard</span><a href="/day-view">Day View <ArrowUpRight size={15}/></a><a href="/calendar">Calendar <ArrowUpRight size={15}/></a><a href="/insights">Insights <ArrowUpRight size={15}/></a><a href="/ai">Ask Soma <ArrowUpRight size={15}/></a></nav><p className={styles.sideNote}>A little structure.<br/>Room to focus.</p></aside>}
    <main className={styles.main}>
      ...
      <div className={styles.columns}><div><section className={styles.section}><div className={styles.sectionHeading}><h2>{day===0 ? 'Today’s plan' : 'Your plan'}</h2><div className={styles.planTools}>{editing && <button onClick={()=>openEditor()}>+ Add block</button>}<button aria-pressed={editing} onClick={()=>{setEditing(!editing);setEditor(null);}}>{editing ? 'Done editing' : 'Edit plan'}</button></div></div><p className={styles.description}>A place for your work, and space between it.</p>
